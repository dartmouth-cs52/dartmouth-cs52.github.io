...
Wait, but where do the unique note `id`s come from?  Eventually they'll be generated by Firebase, but for now we can just use an integer counter.  This is not generally good practice because if you have clashes in keys you can and will get weird behavior as react can't tell the elements apart.
...
🚀 Create a file,  `firebasedb.js` in your `src/` directory. And since we're using npm to fetch the firebase SDK for us, just do `import firebasedb from 'firebasedb';` and you're all set to go!
