---
layout: page
title: Server Starter-Pack — Short Assignment
published: false
comment_term: sa-server-starterpack
---
![](img/enm.jpg){: .small }

## Overview

Today we're going to make a server starter pack similar to our frontend starter.  It will be an [express](https://expressjs.com/) server that displays some data which is stored using [mongodb](https://www.mongodb.com/). We'll get a taste of how to use simple server side routes for displaying some data from a database, and we will gain some knowledge about [express](https://expressjs.com/).
...
We'll build a server starter package that will use:
...
🚀 To start grab the github classroom link to start a new empty repository.


### start a node project

🚀 Remember how to start a brand new node project?
...
All this did was create a barebones `package.json`  file.  This file defines a node.js based project and is required.
...
Remember, this file adds in support for new es6/7 syntax.

### Add in Eslint

```bash
yarn add eslint eslint-config-airbnb eslint-plugin-import
```
